...
keyword_number = int(input("Enter how many keywords you want to search for each topic(0為找全部關鍵字的意思):"))
...
for i in range(len(key_list)):
    if(keyword_number)>0:
        index_and_metrics["index{0}".format(i)],index_and_metrics["metrics{0}".format(i)] =  model.cosine(u'%s' %key_list[i],keyword_number-1)
    else:
        index_and_metrics["index{0}".format(i)],index_and_metrics["metrics{0}".format(i)] =  model.cosine(u'%s' %key_list[i],1000000)
...
#前面為 For小組作業一 的作品(最後小組沒採用)
#以下為 為了hw2新增的東西
def get_relatedword(path):
    keywords = pd.read_excel(path,"L2_foxconn_keyword")['term']
    return keywords
relatedword_list = get_relatedword(path)

indexList=[]
count = 1
for k in range(len(key_list)):
    for i in range(len(relatedword_list)):
        if relatedword_list[i] in model.vocab:
            index = np.where(model.vocab == relatedword_list[i])
            indexList.append(index[0][0])
    indexList.sort()
    for i in indexList:
        print("主題"+key_list[k] + "的第"+ str(count) +"相關詞為" + model.vocab[i])
        count += 1
  # 按照cosine由高往低排列
